...
runCommandViaGur () {
  command=$1
  echo "  running command $1 via gur (TODO: need to run this on the remote machine instead of locally)"
  ~/Downloads/gur-darwin-10.6-amd64/gur --exec "$command;" attach ipc:$HOME/dev/ur_data.prod/gur.ipc
}

getGurPeers () {
  node=$1
  echo "  $1 has the following peers:"
  runCommandViaGur admin.peers | grep "^ *remoteAddress:" | cut -f 2 -d\"
}
